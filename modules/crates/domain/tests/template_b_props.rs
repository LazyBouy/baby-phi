//! Property tests for Template B adoption builder (M3/P2 commitment C6).
//! Same invariants as Template A but with the `#template:b` tag and
//! `org:<id>/template:b` resource URI.
...
use domain::templates::b::{build_adoption_request, AdoptionArgs};
...
proptest! {
    #[test]
    fn template_b_adoption_is_always_approved(ceo in arb_ceo()) {
    ...
    #[test]
    fn template_b_carries_template_b_kind_tag(ceo in arb_ceo()) {
        let ar = build_adoption_request(AdoptionArgs {
            org_id: OrgId::new(),
            ceo,
            now: Utc::now(),
        });
        prop_assert!(ar.kinds.contains(&"#template:b".to_string()));
    }

    #[test]
    fn template_b_resource_uri_encodes_org_id(org_id_seed in any::<u128>()) {
        ...
        let expected_uri = format!("org:{}/template:b", org_id);
        prop_assert_eq!(&ar.resource_slots[0].resource.uri, &expected_uri);
    }

    #[test]
    fn template_b_serde_round_trip_is_identity(ceo in arb_ceo()) {
        let ar = build_adoption_request(AdoptionArgs {
            org_id: OrgId::new(),
            ceo,
            now: Utc::now(),
