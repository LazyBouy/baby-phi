//! Property tests for Template C adoption builder (M3/P2 commitment C6).
...
use domain::templates::c::{build_adoption_request, AdoptionArgs};
...
proptest! {
    #[test]
    fn template_c_adoption_is_always_approved(ceo in arb_ceo()) {
    ...
    #[test]
    fn template_c_carries_template_c_kind_tag(ceo in arb_ceo()) {
        let ar = build_adoption_request(AdoptionArgs {
            org_id: OrgId::new(),
            ceo,
            now: Utc::now(),
        });
        prop_assert!(ar.kinds.contains(&"#template:c".to_string()));
    }

    #[test]
    fn template_c_resource_uri_encodes_org_id(org_id_seed in any::<u128>()) {
        ...
        let expected_uri = format!("org:{}/template:c", org_id);
        prop_assert_eq!(&ar.resource_slots[0].resource.uri, &expected_uri);
    }

    #[test]
    fn template_c_serde_round_trip_is_identity(ceo in arb_ceo()) {
        let ar = build_adoption_request(AdoptionArgs {
            org_id: OrgId::new(),
            ceo,
            now: Utc::now(),
