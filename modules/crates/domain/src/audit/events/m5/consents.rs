...
//!
//! Five `Logged`-class builders, one per transition that the per-transition
//! Repository methods invoke:
//!
...
//! `org_scope` populated from `consent.scope.org`.

use chrono::{DateTime, Utc};

use crate::audit::{AuditClass, AuditEvent};
use crate::model::ids::{AgentId, AuditEventId, ConsentId, NodeId, OrgId};
