...
- **C-M5-6 — Shape B materialisation-after-both-approve (discovered at M4/P6 close).** M4/P6 ships the full Shape B Auth-Request machinery — 2-approver slot AR created at submit, `approve_pending_shape_b` drives each slot through `transition_slot`, the 4-outcome decision matrix (both-approve / both-deny / mixed A-D / mixed D-A) is pinned by `shape_b_approval_matrix_props` (50-case proptest at the domain tier) AND by `acceptance_projects_create.rs` scenarios at the HTTP tier. What M4 does **NOT** ship: the Approved terminal branch's actual project-materialisation call. The HTTP response on both-approve currently returns `{outcome: "terminal", state: "approved", project_id: null}` with an audit emit for `platform.project_creation.denied` suppressed on the Approved path. Reason: `approve_pending_shape_b` runs in a handler that has only the `AuthRequestId` — the full `CreateProjectInput` (name, goal, leads, members, OKRs, token_budget, resource_boundaries) was captured at submit time but not persisted anywhere the approve handler can read back. Two persistence options were considered at M4/P6: (a) serde-encode the payload into the AR's `justification` string (pragmatic; conflates human-readable reason with machine data; chosen against), (b) dedicated `shape_b_pending_projects` sidecar table keyed by AR id (proper; requires migration 0005; scheduled for M5). M5 wires: (a) new migration `0005_*.surql` adding `shape_b_pending_projects` table with `auth_request_id UNIQUE` + `payload FLEXIBLE TYPE object` columns; (b) `Repository::persist_shape_b_pending` + `fetch_shape_b_pending` + `delete_shape_b_pending` repo methods; (c) Shape B submit writes the sidecar row inside the same transaction as `create_auth_request`; (d) `approve_pending_shape_b` Approved branch reads the sidecar, calls `materialise_project`, deletes the sidecar on success, emits `platform.project.created` + fires `DomainEvent::HasLeadEdgeCreated`; (e) acceptance test `shape_b_both_approve_materialises_project` flips from asserting `project_id: null` to asserting a real project id + verifying the `has_lead_edge` exists post-materialisation. **Files affected at M5:** `store/migrations/0005_*.surql` (sidecar table), `domain/src/repository.rs` (3 new methods + domain type `ShapeBPendingProject`), `server/src/platform/projects/create.rs` (un-defer the materialise_project call in `approve_pending_shape_b`; remove the `_keep_materialise_live` hack), `server/tests/acceptance_projects_create.rs` (upgrade the both-approve assertion). **phi-core leverage implications**: none — this is pure phi governance plumbing. The deferred `materialise_project` function at M4/P6 already takes the full `CreateProjectInput`; M5 just wires the reconstruction path. See `m4/architecture/shape-a-vs-shape-b.md` §"Materialisation-after-approve" for the M4-side scaffolded-but-gated code path.

- **C-M5-5 — Per-agent `ModelConfig` binding + active-session gating activation (discovered at M4/P5 close).** M4/P5 wired the page-09 profile editor with every field the plan prescribed *except* `ModelConfig` edits — phi-core's `AgentProfile` has no `model_config` field (only a `config_id` that's a stable identity for `loop_id` composition, not a provider reference), and baby-phi's governance `AgentProfile` wrap did not add a per-agent model-binding extension. The supporting 409 `ACTIVE_SESSIONS_BLOCK_MODEL_CHANGE` code path + the `Repository::count_active_sessions_for_agent` stub (returning `Ok(0)` at M4) ship today so M5 flips one predicate rather than restructuring the update orchestrator. M5 wires: (a) a new field `AgentProfile.model_config_id: Option<String>` on baby-phi's `AgentProfile` (governance extension on top of the phi-core `blueprint` wrap), with migration `0005_*.surql` adding the nullable column + `#[serde(default)]` for backward-compat deserialisation of pre-M5 rows; (b) validation in `server/src/platform/agents/update.rs` that the submitted `model_config_id` references a row in the org's `ModelRuntime` catalogue (M2 surface); (c) replacement of the `count_active_sessions_for_agent` stub with a real query joining `Session` (C-M5-3) + agent_id, filtered to non-terminal states; (d) acceptance test that triggers the 409 by launching a session (C-M5-3 primitive), then attempting a `model_config_id` PATCH, then asserting the error code + message. **Files affected at M5:** `domain/src/model/nodes.rs::AgentProfile` (add `model_config_id` field), `store/migrations/0005_*.surql` (column add), `server/src/platform/agents/update.rs` (re-enable the ModelConfig change arm that was commented out at M4/P5), `store/src/repo_impl.rs::count_active_sessions_for_agent` (flip from stub to SurrealQL query), `server/tests/acceptance_agents_profile.rs` (add `model_config_change_on_active_session_returns_409` scenario). **phi-core leverage implications**: `phi_core::provider::model::ModelConfig` becomes a Q1 direct import at M5 for the catalogue validation path (was deferred from M4 per this commitment). Zero wrap needed — the reference is an opaque `String` id that keys into the M2 `ModelRuntime` table. Consult the M4/P5 `agent-profile-editor.md` §"Deferred to M5" bullet list. See also M4 plan `m4-agents-and-projects-a634be65.md` §"Part 4 — P5 deliverables" for the M4-side stub wiring (error-code variant + repo method) that M5 flips.
