...
**Base plan entry**: [§M5](/root/projects/phi/baby-phi/docs/specs/plan/build/build-plan-v01-36d0c6c5.md) — 4 lines of scope + six M5 carryovers. This plan is the fully-resolved version.
...
| C26 | **C-M6-1 pinned in base build plan** (new — from D6 user clarification) | Memory node tier + interface contract any memory system must implement + ownership-via-multi-tag (agent / group / project / org; one memory can carry multiple tags) + permission-over-time retrieval (agents retrieve only what they have current grants for) + default implementation shipping alongside the contract | P0 | Amendment landed in [`/root/projects/phi/baby-phi/docs/specs/plan/build/build-plan-v01-36d0c6c5.md`](/root/projects/phi/baby-phi/docs/specs/plan/build/build-plan-v01-36d0c6c5.md) §M6 as new subsection `#### Carryovers from M5`; verified by `grep -n 'Carryovers from M5' <file>` returning 1 hit; doc-link check green. |
...
6. **Base plan amendment — exact file to edit: [`/root/projects/phi/baby-phi/docs/specs/plan/build/build-plan-v01-36d0c6c5.md`](/root/projects/phi/baby-phi/docs/specs/plan/build/build-plan-v01-36d0c6c5.md).** Inside its §M6 subsection (header `### M6 — Agent self-service surfaces (≈2 weeks)`), add a new subsection `#### Carryovers from M5 — must-pick-up at M6 detailed planning` immediately after the §M6 header. Mirror the shape of the existing M3→M5 and M4→M5 carryover subsections in that file (lines 268–291 for reference). Content:
   - **C-M6-1** — Memory node tier + interface contract + ownership-by-multi-tag + permission-over-time retrieval. Full text per D6 resolution (Part 3). Files affected at M6: `modules/crates/domain/src/model/nodes.rs::Memory`, new `modules/crates/domain/src/memory_contract.rs` (trait + default impl), `modules/crates/store/migrations/0006_*.surql` (memory table + `HAS_MEMORY` edges + tag-index table), `modules/crates/server/src/platform/memory/` new dir, `modules/crates/cli/src/commands/memory.rs` (`phi memory {list, show, tag, retrieve}`). P0 close runs `grep -n 'Carryovers from M5' docs/specs/plan/build/build-plan-v01-36d0c6c5.md` → expects 1 hit to confirm the amendment landed.
...
8. **Base plan M6 carryover confirmation** — re-verify the C-M6-1 entry still exists in [`/root/projects/phi/baby-phi/docs/specs/plan/build/build-plan-v01-36d0c6c5.md`](/root/projects/phi/baby-phi/docs/specs/plan/build/build-plan-v01-36d0c6c5.md) §M6 `#### Carryovers from M5` subsection + structured with (i)/(ii)/(iii) per D6 resolution. If a future commit between P0 and P9 accidentally dropped or edited it, flag as HIGH regression.
...
- ✅/⚠/✗ M6 carryover C-M6-1 still present in the base plan at `/root/projects/phi/baby-phi/docs/specs/plan/build/build-plan-v01-36d0c6c5.md` §M6 (re-verify — this is where drift from a P0-planted amendment would show up after 9 phases of churn).
...
| C26 | C-M6-1 pinned in base plan | `docs/specs/plan/build/build-plan-v01-36d0c6c5.md` §M6 amendment landed at P0 |
...
- [`/root/projects/phi/baby-phi/docs/specs/plan/build/build-plan-v01-36d0c6c5.md`](/root/projects/phi/baby-phi/docs/specs/plan/build/build-plan-v01-36d0c6c5.md) — §M6 amendment at P0: new subsection `#### Carryovers from M5 — must-pick-up at M6 detailed planning` with **C-M6-1** (Memory contract + ownership-by-multi-tag + permission-over-time). Immediately after the existing `### M6 — Agent self-service surfaces (≈2 weeks)` header; mirrors the existing M3→M5 + M4→M5 + M4→M8 carryover subsection shapes in the same file.
