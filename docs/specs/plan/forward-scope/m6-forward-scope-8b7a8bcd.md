<!-- Last verified: 2026-05-24 by Claude Code (CH-28c inline-draft authoring per chunk-initiate Option A — phi-core 0.9.0 absorption chunk (per-turn debug capture + async-trait migration; baseline shift) row added as prerequisite commit before chunk-planner spawn; §1 Foundation tier extended with CH-28c TECHNICAL-PREREQUISITE block between CH-28b and CH-29; §4 dep graph ASCII art + critical-path chain + foundation parallel band wording amended to insert CH-28c node between CH-28b close and the foundation parallel band (CH-29 + CH-31 + CH-32 + CH-38 now post-CH-28c unblock); §5 summary table CH-28c row added with v2 Chunk-type + User-visible delivery columns; §5 totals bumped 12 → 13 chunks, upper-band 39 → 39.5 ed, lower-band 28.5 → 28.8 ed, distribution 6 FUNCTIONAL + 7 TECHNICAL-PREREQUISITE (CH-28 + CH-28b + CH-28c + CH-29 + CH-30 + CH-31 + CH-38); §5 chain-cap evaluation note extended to reflect CH-28 → CH-28b → CH-28c → CH-29 → CH-30 → CH-33 = 5 consecutive TECHNICAL-PREREQUISITE (still absorbed by CH-32 FUNCTIONAL parallel-band delivery). Plan archive: ad-hoc plan-mode session 2026-05-24 (file `/root/.claude/plans/sharded-discovering-stearns.md`; chunk plan archives separately at `docs/specs/plan/build/<slug>-<8hex>/plan.md` post chunk-planner spawn). Continues CH-NNa/b/c interstitial suffix pattern established by CH-28b for phi-core absorption chunks. Per chunk-planner v26 + phase-planner v2 chunk-type tagging.) -->
<!-- Last verified: 2026-05-23 by Claude Code (CH-28b inline-draft authoring per chunk-initiate Option A — phi-core 0.8.0 absorption chunk (Composition I baseline shift) row added as prerequisite commit before chunk-planner spawn; §1 Foundation tier extended with CH-28b TECHNICAL-PREREQUISITE block between CH-28 and CH-29; §4 dep graph ASCII art + critical-path chain + foundation parallel band wording amended to insert CH-28b node between CH-28 close and the foundation parallel band (CH-29 + CH-31 + CH-32 + CH-38 now post-CH-28b unblock); §5 summary table CH-28b row added with v2 Chunk-type + User-visible delivery columns; §5 totals bumped 11 → 12 chunks, upper-band 38 → 39 ed, lower-band 28 → 28.5 ed, distribution 6 FUNCTIONAL + 6 TECHNICAL-PREREQUISITE; §5 chain-cap evaluation note extended to reflect CH-28 → CH-28b → CH-29 → CH-30 → CH-33 = 4 consecutive TECHNICAL-PREREQUISITE (still absorbed by CH-32 FUNCTIONAL parallel-band delivery). Plan archive: ad-hoc plan-mode session 2026-05-23 (file `/root/.claude/plans/sharded-discovering-stearns.md`; chunk plan archives separately at `docs/specs/plan/build/<slug>-<8hex>/plan.md` post chunk-planner spawn). Files CH-28b filed as TECHNICAL-PREREQUISITE chunk per chunk-planner v26 + phase-planner v2 chunk-type tagging.) -->
...
**CH-28b — phi-core 0.8.0 absorption (Composition I braking layer; baseline shift)** · MEDIUM · 0.5–1d
- **Chunk-type**: TECHNICAL-PREREQUISITE
- **Functional outcome**: NONE this chunk — technical prerequisite for ALL downstream M6 chunks (CH-29..CH-38). Shifts baby-phi's phi-core dependency baseline from 0.7.1 to 0.8.x to absorb the Composition I breaking-change surface released 2026-05-23 (phi-core's first crates.io publish since baby-phi pinned 0.7.1 at M5 close). Non-technical-user rationale: *"The library baby-phi sits on top of (phi-core) released a new version 0.8.0 today with a small breaking change; we absorb it now so the rest of M6 runs on the up-to-date baseline + future phi-core 0.8.x patches flow through automatically."* User observes no behavior change at CH-28b close — Composition I is opt-in via `with_revert_tool()` which baby-phi does NOT enable this chunk.
- **Defers (with product impact)**: Composition I adoption (`.with_revert_tool()` call site + RevertRenderPolicy tuning + revert-discipline skill teaching the agent the four categories failure/tangent/completion/step-summary) → future M6+ FUNCTIONAL chunk (no specific CH-NN slot reserved yet; tracked via `D-PHICORE-08-FOLLOWUP-01`). **User-visible impact while deferred**: agents cannot yet abandon failed/finished conversation branches between turns to keep active context lean; context still grows monotonically per the pre-0.8.0 posture (compaction is the only relief). After future Composition I adoption: agents emit `revert_to_state` tool calls between turns to drop dead branches; context stays lean; compaction fires less often.
- **Drifts closed**: NONE closed; files NEW `D-PHICORE-08-FOLLOWUP-01` for Composition I adoption deferral.
- **Concept docs**: external — [phi-core CHANGELOG §[0.8.0]](../../../../phi-core/CHANGELOG.md) (breaking-change enumeration); [phi-core `concept-brake.md` §5 Composition I](../../../../phi-core/docs/concepts/concept-brake.md) (design source of truth); [i-phi plan archive `phi-core-revert-tool-27c894f6/plan.md` §DIAGNOSTIC SECTION A](../../../../i-phi/docs/v0/proposal/plan/build/phi-core-revert-tool-27c894f6/plan.md) (empirical breaking-change impact for baby-phi).
- **Prerequisites**: none (orthogonal to CH-28's schema redesign; CH-28 is closed at cycle `0412eb06` 2026-05-20).
- **Deliverables**: workspace `Cargo.toml` bump `phi-core = "0.7.1"` → `phi-core = "0.8"` (semver-range per F2.a planner-rec — picks future 0.8.x patches automatically); `cargo update -p phi-core` Cargo.lock update; `AgentLoopConfig.revert_pending: None` field added to the existing struct-literal at `server/src/platform/sessions/launch.rs:526-568` (parallel to CH-25's `response_format: None` repair); explicit `AgentEvent::RevertApplied { .. } => {}` arms added at `domain/src/session_recorder.rs:166-170` + `cli/src/commands/agent.rs:642-669` for future-reader discoverability (existing wildcards `..` / `_ => {}` already cover the variant; explicit arms are cosmetic but signal awareness); NEW drift file `D-PHICORE-08-FOLLOWUP-01` at baby-phi's canonical drift directory capturing Composition I adoption-deferral rationale + 4 adoption prerequisites + cross-references to phi-core docs; feature-inventory.md §3 Deferred catalogue row added for the drift; this row in m6-forward-scope is the "row added" deliverable (authored as prerequisite at chunk-open per chunk-initiate Option A — collapses chunk's own D7 to verified-header amendment).
- **K8s posture**: zero K8s blocker class delta (no new migration; no new in-process state; no new IPC; no new trait surface; no new cross-pod state; no new audit hash-chain edge).
- **phi-core leverage**: dependency baseline shift only; no new phi-core import surface this chunk. Composition I types (`NodeId`, `NodeTag`, `RevertCategory`, `RevertRequest`, `RevertTool`, `RevertApplied` event) become available in baby-phi's import space but ARE NOT imported until the future adoption chunk fires. Existing imports of `phi_core::types::event::AgentEvent` continue to compile via the existing wildcard match arms.
- **Unblocks**: ALL downstream M6 chunks (CH-29..CH-38) by virtue of the workspace compiling on the 0.8.x baseline; in particular CH-33 (first FUNCTIONAL M6 chunk that consumes `phi_core::types::tool::AgentTool` for `read_inbox` + `send_message` tools — but the consumption surface is unchanged across 0.7.1 → 0.8.0 per phi-core CHANGELOG); future Composition I adoption chunk (when scheduled).

**CH-28c — phi-core 0.9.0 absorption (per-turn debug capture + async-trait migration; baseline shift)** · LOW · 0.3–0.5d
- **Chunk-type**: TECHNICAL-PREREQUISITE
- **Functional outcome**: NONE this chunk — technical prerequisite for ALL downstream M6 chunks (CH-29..CH-38). Shifts baby-phi's phi-core dependency baseline from 0.8 to 0.9.x to absorb the phi-core 0.9.0 release shipped 2026-05-24 (per-turn debug capture surface + broad async-trait migration of `BlockCompactionStrategy` + 9 of 11 `AgentLoopConfig` lifecycle Fns + `InputFilter`). Non-technical-user rationale: *"phi-core released another minor version one day after 0.8.0 with two added capabilities — per-turn debug capture (currently dormant; opt-in) and an async upgrade to the agent loop's internal hook points (currently unused by baby-phi). We absorb the version baseline shift today so the rest of M6 runs on 0.9.x + future patches flow through automatically. Even smaller surface than CH-28b: literally ZERO compile breaks."* User observes no behavior change at CH-28c close — per-turn debug capture is opt-in via `SessionRecorderConfig::capture_turn_requests` (default `false`) which baby-phi does NOT enable this chunk; async-trait migration is internal-only to phi-core + has no impact on baby-phi consumers (zero lifecycle hooks wired; zero `BlockCompactionStrategy` impls; zero `InputFilter` impls).
- **Defers (with product impact)**: per-turn debug capture adoption (`capture_turn_requests: true` flag flip + `Turn::request_payload` telemetry surfacing + JSON-size-implications documentation) → future M7+ observability chunk (no specific CH-NN slot reserved; tracked via `D-PHICORE-09-FOLLOWUP-01`). **User-visible impact while deferred**: agents' fully-assembled LLM request payloads (system prompt + post-`convert_to_llm()` `Vec<Message>` array + tool definitions + parallel-indexed `BlockProvenance` vec) are NOT persisted to disk; debugging an unexpected LLM response requires reconstructing the input context manually from `Turn.input_messages` + ad-hoc re-application of `convert_to_llm()`. After future per-turn debug capture adoption: the exact wire payload is in `Turn.request_payload` for every captured turn (opt-in per agent / per cycle to keep session JSON size bounded). Async-trait migration deferral: N/A — baby-phi never used the 9 lifecycle hooks at sync, so async-fication is a no-op consumer-side; no migration work needed at adoption time either.
- **Drifts closed**: NONE closed; files NEW `D-PHICORE-09-FOLLOWUP-01` for per-turn debug capture adoption deferral.
- **Concept docs**: external — [phi-core CHANGELOG §[0.9.0]](../../../../phi-core/CHANGELOG.md) (breaking-change enumeration); [phi-core `debugging.md`](../../../../phi-core/docs/concepts/debugging.md) (per-turn debug capture design source of truth); [i-phi plan archive `phi-core-0.9.0/plan.md` §"baby-phi absorption"](../../../../i-phi/docs/v0/proposal/plan/build/phi-core-0.9.0/plan.md) (empirical breaking-change impact for baby-phi; lines 267-273).
- **Prerequisites**: **CH-28b** (must close first; the 0.8 → 0.9 bump assumes baby-phi is already on 0.8 baseline post-CH-28b cycle `d5b776ac` commit `01e59f2` 2026-05-23).
- **Deliverables**: workspace `Cargo.toml` bump `phi-core = "0.8"` → `phi-core = "0.9"` (semver-range per F2.a planner-rec); `cargo update -p phi-core` Cargo.lock update; explicit `AgentEvent::TurnRequest { .. } => {}` arm added at `cli/src/commands/agent.rs:~677` for future-reader discoverability (parallels CH-28b's explicit `RevertApplied` arm); session_recorder.rs `matches!()` single-pattern site intentionally NOT modified per scope-narrow (same rationale as CH-28b D4); NEW drift file `D-PHICORE-09-FOLLOWUP-01-per-turn-debug-capture-adoption.md` at canonical `m6/drifts/`; feature-inventory.md §3 Deferred catalogue row added for the drift; NEW ADR-0065 with ~5 sub-decisions §D65.1–§D65.5; this row in m6-forward-scope is the "row added" deliverable (authored as prerequisite per chunk-initiate Option A — collapses chunk's own forward-scope deliverable to verified-header amendment only at chunk-close).
- **K8s posture**: zero K8s blocker class delta (no new migration; no new in-process state; no new IPC; no new trait surface; no new cross-pod state; no new audit hash-chain edge).
- **phi-core leverage**: dependency baseline shift only; no new phi-core import surface this chunk. Per-turn debug capture types (`BlockProvenance`, `ProvenanceRole`, `AnnotatedRequestPayload`, `AgentEvent::TurnRequest`) become available in baby-phi's import space but ARE NOT imported until the future adoption chunk fires. Async-trait migration of `BlockCompactionStrategy` + 9 lifecycle Fns + `InputFilter` is internal-only to phi-core's trait shapes; baby-phi has no consumers of any of these surfaces (verified via grep) so async-fication is a no-op consumer-side.
- **Unblocks**: ALL downstream M6 chunks (CH-29..CH-38) on the 0.9.x baseline; in particular CH-33 (first FUNCTIONAL M6 chunk consuming `phi_core::types::tool::AgentTool` — the AgentTool surface is unchanged across 0.8 → 0.9 per CHANGELOG); future per-turn debug capture adoption chunk (when scheduled at M7+ observability).
...
```
                ...
                CH-28 — AgentProfile cardinality 1:1 → N:1 (Q9)
                              │
                              v
                CH-28b — phi-core 0.8.0 absorption (Composition I baseline shift)
                              │
                              v
                CH-28c — phi-core 0.9.0 absorption (per-turn debug capture + async-trait migration)
                              │
        ┌─────────────────────┼───────────────────┬───────────────────┐
        v                     v                   v                   v
...
`CH-28 → CH-28b → CH-28c → CH-29 → CH-30 → CH-33 → {CH-34, CH-35, CH-36}` (a04 + a05 + a03 + a02 fan-out post-a01).

Side-chain: `CH-31 → CH-37` (embedding provider gates a05 per Q4 user-lock).

### Parallelizable bands

- **Foundation parallel band (post-CH-28c)**: CH-29 + CH-31 + CH-32 + CH-38 can run concurrently (no inter-dependencies; CH-28c unlocks all four — workspace must compile on phi-core 0.9.x before any of them open).
...
- **M6 in-milestone scope** (CH-28 + CH-28b + CH-28c + CH-29..CH-38, 13 chunks): **28.8–39.5 engineer-days** sum of upper-bands.
- **Critical-path serial walk**: CH-28 (4d) + **CH-28b (1d)** + **CH-28c (0.5d)** + CH-29 (3.5d) + CH-30 (3.5d) + CH-33 (4d) + max(CH-34/35/36) (4d) + CH-37 (4d) = **~24.5d serial**. With parallelization of foundation tier + post-a01 fan-out, achievable in **~12–16 calendar working days assuming continuous engineer attention** (effort total still ~28.8–39.5 ed; parallelization compresses calendar, not effort).
...
| CH-28b | phi-core 0.8.0 absorption (Composition I baseline shift) | TECHNICAL-PREREQUISITE | unblocks CH-29..CH-38 (workspace compiles on phi-core 0.8.x; Composition I types become available but are NOT imported until a future adoption chunk fires) | MEDIUM | 0.5–1d | external — phi-core CHANGELOG §0.8.0 + concept-brake.md §5 + i-phi plan archive `phi-core-revert-tool-27c894f6/plan.md` §A | — | yes |
| CH-28c | phi-core 0.9.0 absorption (per-turn debug capture + async-trait migration) | TECHNICAL-PREREQUISITE | unblocks CH-29..CH-38 (workspace compiles on phi-core 0.9.x; per-turn debug capture types + async lifecycle hook signatures become available but are NOT imported / wired until a future adoption chunk fires) | LOW | 0.3–0.5d | external — phi-core CHANGELOG §0.9.0 + concept-doc `debugging.md` + i-phi plan archive `phi-core-0.9.0/plan.md` §baby-phi-absorption | **CH-28b** | yes |
| CH-29 | M6-DEFERRED-02 messaging substrate | TECHNICAL-PREREQUISITE | unblocks CH-30 routing + CH-33 a01 (AgentMessage value-object + Inbox/Outbox row materialization; no cross-agent delivery yet) | HIGH | 2.5–3.5d | ontology.md L83–87 + L229, permissions/05 L590–618, agent.md L21 | CH-28c | yes |
...
**Total upper-band: 39.5 engineer-days. Total lower-band: 28.8 engineer-days. Range: 28.8–39.5 ed for M6 close (13 chunks).**

**Chunk-type distribution**: 6 FUNCTIONAL (CH-32 + CH-33 + CH-34 + CH-35 + CH-36 + CH-37) + 7 TECHNICAL-PREREQUISITE (CH-28 + CH-28b + CH-28c + CH-29 + CH-30 + CH-31 + CH-38).

**Chain-cap evaluation (per phase-planner v2 max-2-consecutive-TECHNICAL-PREREQUISITE rule)**: serial messaging axis CH-28 → **CH-28b** → **CH-28c** → CH-29 → CH-30 → CH-33 is **5 consecutive TECHNICAL-PREREQUISITE** before FUNCTIONAL CH-33 lands (CH-28b inserted 2026-05-23 for phi-core 0.8.0 baseline shift; CH-28c inserted 2026-05-24 for phi-core 0.9.0 baseline shift) → APPARENT violation widens by 2 vs original. However, **CH-32 FUNCTIONAL runs in PARALLEL** with CH-29 + CH-30 + CH-31 (per §4 dep graph foundation band, post-CH-28c unblock) — so the user observes FUNCTIONAL delivery (memory-recall CLI + HTTP) DURING the messaging-axis prerequisite chain. Net: the parallel-band scheduling absorbs the chain-cap concern even at 5-consecutive. Surfaced as §6 Open Question Q-CHAIN-1 for explicit user lock at CH-29 plan-open. Both CH-28b (0.5–1d) and CH-28c (0.3–0.5d) are low-effort baseline-shift chunks that do not materially expand the chain-cap risk; the dominant factor remains messaging-axis serialization which the parallel band already addresses.
