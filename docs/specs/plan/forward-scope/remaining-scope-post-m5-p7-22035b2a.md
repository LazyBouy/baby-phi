<!-- Last verified: 2026-05-19 by Claude Code (CH-28 P-SEAL — §3 M6+-OPEN-01 marker Resolution line authored at L371: resolved at CH-28 / ADR-0063 with F1.c hybrid Blueprint table + F2.b USES_PROFILE rename + F3.b split migrations (0019 schema + 0020 backfill); concept-doc amendments landed at CH-28 P-DOCS in agent.md §Soul + ontology.md L98 cardinality flip + (74 total) header bump; NEW architecture doc m6/architecture/agent-profile-cardinality.md authored; D-CH28-FOLLOWUP-01 drift filed for template-tier fan-out scope-narrowing to M6-DEFERRED-04 / CH-36. CH-28 is the first M6 chunk to close; cumulative cross-cycle divergent forks now 14-of-19 (~74%). Cycle hex `0412eb06`.) -->
<!-- Last verified: 2026-05-18 by Claude Code (CH-27 P-SEAL — §2.5 amended: CH-27 row CLOSED (cycle hex `0edcaba9` 2026-05-18; D-CH26-FOLLOWUP-01 transitions discovered → remediated at 3-of-4 axes per ADR-0062 §D62.1+§D62.2+§D62.4; resolvers-wiring axis deferred to M6 via NEW D-CH27-FOLLOWUP-01). M5.3 carve-out closes with 3-chunk arc {CH-25, CH-26, CH-27}; M6 plan-mode now unblocks. ADR-0062 Accepted with 6 sub-decisions. Count-amendment per §D62.5 META applied at body level + footnoted ("15" → "7" advisory check_permission invocations). F4.b USER-DIVERGENT user-lock cumulative cross-cycle pattern now 11-of-16 (~69%, softening from prior ~83%). Cycle hex `0edcaba9`.) -->
...
### CH-27 — Blocking-gate enforcement + resolvers wiring + acceptance fixture extension (M5.3 carve-out extension) — **CLOSED at cycle hex `0edcaba9` 2026-05-18**

- **Drifts closed**: **D-CH26-FOLLOWUP-01** (LOW; CH-27 P-SEAL ✓; 3-of-4 axes closed: wire-tier + synth-grant + acceptance-fixture; resolvers-wiring axis deferred to M6 via NEW D-CH27-FOLLOWUP-01).
- **NEW follow-up drift filed at chunk-seal**: [`D-CH27-FOLLOWUP-01`](../../v0/implementation/m5_3/drifts/D-CH27-FOLLOWUP-01.md) (LOW; M6-DEFERRED-RESOLVERS-WIRING per ADR-0062 §D62.3 F3.a LOCKED).
- **Concept docs**: `permissions/04-manifest-and-resolution.md` (synth-grant widening to 4-verb scope `[Allocate, Transfer, Observe, Inspect]` for owner-Agents); `m5_3/architecture/composite-resources-model.md` (tightened advisory → blocking + NEW §"Test-fixture pattern for owner-grant-required tests"); `m5_3/architecture/agent-ownership-model.md` (synth-grant scope subsection refreshed); `m5_3/operations/composite-resources-operations.md` (blocking-gate operator narrative).
- **Prerequisites**: **CH-26** ✓ (advisory infrastructure shipped at CH-26 P-SEAL).
- **Deliverables (shipped)**:
  - **§D62.1 — Wire-tier tightening (cardinality amended per §D62.5)**: 7 advisory `check_permission` invocations[^count-fix] flipped to blocking gates (engine-deny → HTTP 403 `NO_GRANTS_HELD` via `denial_to_api_error`). The original "15" claim conflated invocations + use imports + docstring references; verified count is **7** (one production-call per handler).
  - **§D62.2 — Synth-grant widening**: CH-25 synth-owner-grant rule (`step_2_resolve_grants`) widened from `[Allocate, Transfer]` (CH-25 baseline) to `[Allocate, Transfer, Observe, Inspect]` (4 universal-applicability verbs per `concepts/permissions/03-action-vocabulary.md:44`).
  - **§D62.3 — Resolvers wiring DEFERRED to M6 (F3.a LOCKED, user-aligned)**: `projects::resolvers::*` actor-passthrough wiring deferred to **M6-DEFERRED-RESOLVERS-WIRING** via NEW drift [`D-CH27-FOLLOWUP-01`](../../v0/implementation/m5_3/drifts/D-CH27-FOLLOWUP-01.md).
  - **§D62.4 — F4.b USER-DIVERGENT fixture extension**: NEW opt-in helper `seed_owner_grants` at [`server/tests/acceptance_common/owner_grants.rs`](../../../modules/crates/server/tests/acceptance_common/owner_grants.rs). **9 explicit call-sites** across 6 M3+M4+M5 acceptance test files (`acceptance_m3.rs`, `acceptance_m4.rs`, `acceptance_m5_orgs.rs`, `acceptance_orgs_create.rs`, `dashboard_silent_filter_test.rs`, `show_organization_post_filter_test.rs`) get explicit per-test owner-grant seeding. Planning band per plan §3 Artifact C was 12-18; cascade-collapse documented in ADR-0062 §D62.4 — tests using the `apply_org_creation` production path obtain `Edge::Owns` implicitly per CH-25 ADR-0060 §D60.1.
  - **2 advisory-only-renamed acceptance tests re-enabled**: `owner_inspect_via_show_organization_handler_path` + `owner_observe_via_dashboard_handler_path` at `acceptance_m5_3_composite_resources.rs` (renamed at P2, extended with HTTP-tier 200-pass assertions at P3).
  - **4 NEW HTTP 403-block scenarios** at `acceptance_m5_3_composite_resources.rs`: `unauthorized_actor_blocked_at_{show_organization,org_dashboard,project_detail,set_agent_supervisor}_returns_403`.
  - **NEW ADR**: [`ADR-0062`](../../v0/implementation/m5_3/decisions/0062-blocking-gate-and-synth-grant-widening.md) Accepted with 6 sub-decisions §D62.1-§D62.6.
- **Estimated effort (actual)**: ~7 engineer-days (handler tightening + fixture extension ~3 ed + synth-grant widening ~1 ed + HTTP scenarios + ADR + drift housekeeping ~2-3 ed; F4.b helper SCOPE-NARROWING vs plan §3 Artifact C literal absorbed at P-FIXTURES with explicit-grant materialisation via `Repository::create_grant`).

[^count-fix]: Cardinality amended at CH-27 P-SEAL per ADR-0062 §D62.5 META. The "15" in the CH-26 row above + the original D-CH26-FOLLOWUP-01 body conflated invocations + use imports + docstring references. The verified count is **7** (one production-call per handler at `orgs/{list,show,create,dashboard}.rs` + `projects/{create,detail,agent_supervisor}.rs`).
...
- **Resolution**: Resolved at CH-28 / ADR-0063 (cycle hex `0412eb06`, 2026-05-19) — pursued the N:1 flip with F1.c hybrid blueprint table + F2.b USES_PROFILE rename + F3.b split migrations (0019 schema + 0020 backfill). Concept-doc amendments landed at CH-28 P-DOCS in `concepts/agent.md` §"Soul" lines 160–178 + `concepts/ontology.md` line 98 cardinality flip + (74 total) header bump. NEW architecture doc `m6/architecture/agent-profile-cardinality.md`. See plan archive at [`build/ch-28-agentprofile-cardinality-redesign-0412eb06/plan.md`](../build/ch-28-agentprofile-cardinality-redesign-0412eb06/plan.md) + ADR-0063 for body.
