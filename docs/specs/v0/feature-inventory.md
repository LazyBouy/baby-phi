<!-- Last verified: 2026-05-25 by Claude Code (CH-28d P3 — NEW §3 Deferred catalogue row `D-PHICORE-10-FOLLOWUP-01 — phi-core 0.10.0 new-API adoption cluster (4 axes A/B/C/D)` inserted between D-PHICORE-09-FOLLOWUP-01 and D-CH28-FOLLOWUP-01 rows; documents the phi-core 0.10.0 4-axis new-API adoption deferral (Axis A revert-render-policy wire-through + Axis B current-tool-timeout pause-time introspection + Axis C detect_interpreter pub-shim adoption + Axis D CurrentToolExecution standalone consumption); allocation `M7+-FUTURE-ARCHITECTURE-SHIFT-OR-OBSERVABILITY-ADOPTION` placeholder per F4.a planner-rec lock + ADR-0066 §D66.4; cycle hex `79b7386b`.) -->
<!-- Last verified: 2026-05-24 by Claude Code (CH-28c P3 — NEW §3 Deferred catalogue row `D-PHICORE-09-FOLLOWUP-01 — Per-turn debug capture adoption (phi-core 0.9.0 opt-in observability surface)` inserted between D-PHICORE-08-FOLLOWUP-01 and D-CH28-FOLLOWUP-01 rows; documents the phi-core 0.9.0 opt-in per-turn debug capture adoption deferral (4 prerequisites: capture-flag flip + observability surfacing + JSON-size policy + operator inspection UI); allocation `M7+-FUTURE-PER-TURN-DEBUG-CAPTURE-ADOPTION` placeholder per F3.a planner-rec lock + ADR-0065 §D65.4; cycle hex `40214078`.) -->
...
### D-PHICORE-09-FOLLOWUP-01 — Per-turn debug capture adoption (phi-core 0.9.0 opt-in observability surface)
- **Feature impact**: F-debugging-introspection (post-0.9.0 opt-in per-turn debug capture; surfaces the fully-assembled LLM wire payload — system prompt + post-`convert_to_llm()` `Vec<Message>` array + tool definitions + parallel-indexed `BlockProvenance` vec — for every captured turn).
- **User-visible state in v0 (at CH-28c close, 2026-05-24)**: agents' fully-assembled LLM request payloads are NOT persisted to disk; debugging an unexpected LLM response requires reconstructing the input context manually from `Turn.input_messages` + ad-hoc re-application of `convert_to_llm()`. Per-turn debug capture is opt-in upstream + remains opt-out in baby-phi via `SessionRecorderConfig::default()` at `cli/src/commands/agent.rs:636` (default `capture_turn_requests: false`).
- **User-visible state at final (at future M7+ per-turn debug capture adoption chunk close)**: the exact wire payload is persisted in `Turn::request_payload` for every captured turn (opt-in per agent / per cycle to keep session JSON size bounded); operators run `phi session show --include-request-payload [--turn N]` (or equivalent Web UI panel) to inspect the exact bytes sent to the provider; debugging unexpected LLM responses becomes a one-command lookup. No user-perceived behavior delta beyond operator-tooling ergonomics (faster + more accurate debugging; bounded JSON-size impact per the chosen size policy).
- **Allocation chunk**: M7+-FUTURE-PER-TURN-DEBUG-CAPTURE-ADOPTION (placeholder; no specific CH-NN slot reserved at CH-28c close; future planning session decides whether adoption lands as a dedicated FUNCTIONAL chunk OR bundles into an existing M7 observability chunk that already touches `SessionRecorder` or session-JSON serialization).
- **Cross-chunk dependency**: requires 4 prerequisites (per drift body §"Remediation scope"): (1) `SessionRecorderConfig::capture_turn_requests: true` flag flip (env-var / CLI flag / per-agent config-field toggle); (2) optional `TurnRequest` recorder surfacing in baby-phi's observability layer (audit-event projection for capacity-tracking); (3) JSON-size-implications policy (per-agent toggle / per-cycle toggle / max-payload-bytes truncation / rolling-window retention); (4) operator inspection UI / CLI surface (`phi session show --include-request-payload`).

### D-PHICORE-10-FOLLOWUP-01 — phi-core 0.10.0 new-API adoption cluster (4 axes A/B/C/D: revert-render-policy / current-tool-introspection / detect-interpreter-shim / current-tool-execution-consumer)
- **Feature impact**: F-agent-context-management + F-debugging-introspection + F-hooks-dispatch (post-0.10.0 new-API surfaces: kind-aware Composition I decay-window tag rendering tunability + per-tool-call pause-time introspection + interpreted-script hook dispatch + standalone CurrentToolExecution observation).
- **User-visible state in v0 (at CH-28d close, 2026-05-25)**: agents continue to operate identically to the pre-0.10.0 posture — no kind-aware decay-window tag rendering (Composition I revert mode is disabled per D-PHICORE-08-FOLLOWUP-01 deferral; `revert_render_policy: RevertRenderPolicy::default()` at `launch.rs:592` only takes effect when `active_node_id.is_some()`); no per-tool-call pause-time introspection (`current_tool: None` at `launch.rs:593` disables the introspection slot; baby-phi has no pause/cancel surface at v0); no interpreted-script hook dispatch (baby-phi has no `.sh`/`.py`/`.js` hook-script dispatch path at v0); no standalone `CurrentToolExecution` consumption (the struct flows through only as a generic parameter of the `current_tool: None` field; no source site names the type directly). All 4 new-API axes are opt-in upstream + remain opt-out in baby-phi via the no-op default values + architectural-shift prerequisites (long-lived-agent shape per ADR-0034 §D34.6 OR hook-script dispatch path OR pause/cancel surface).
- **User-visible state at final (at future M7+ architecture-shift OR observability adoption chunk close)**: per-axis closures unlock distinct user-visible features: Axis A close → operator can tune `Lesson`/`Note` decay-window policy via baby-phi config TOML; Axis B close → `PauseAcknowledged.estimated_completion_ms` UI surface shows remaining tool-execution time before pause completes; Axis C close → hooks can dispatch via interpreted scripts (shell/Python/JS) instead of binary executables only; Axis D close → per-tool-call observation surface for operators / debug UIs (audit-event projection emitting `ToolExecutionObserved { session_id, turn_id, tool_name, started_at, elapsed_ms }`). No user-perceived behavior delta beyond the unlocked features (no breaking changes to existing agent behaviour).
- **Allocation chunk**: M7+-FUTURE-ARCHITECTURE-SHIFT-OR-OBSERVABILITY-ADOPTION (placeholder; no specific CH-NN slot reserved at CH-28d close; future planning session decides per-axis routing — Axis C is independently schedulable as a standalone FUNCTIONAL chunk, Axes A+B+D bundle as a single architecture-shift chunk sharing the long-lived-agent prerequisite, Axis A alone is schedulable if the long-lived-agent shift lands without B+D's pause/cancel surface).
- **Cross-chunk dependency**: requires architectural-shift prerequisites (per drift body §"Remediation scope"): (1) Axis A — long-lived-agent shape per ADR-0034 §D34.6 (currently per-request stateless; ~3-5 ed for the shift alone) + Composition I revert mode enablement per D-PHICORE-08-FOLLOWUP-01 closure; (2) Axis B — Axis A prerequisite + pause/cancel surface design (~2-4 ed); (3) Axis C — hook-script dispatch path (independent of Axes A/B/D; ~1-2 ed); (4) Axis D — Axis A + Axis B prerequisites (~0.3-0.6 ed for the audit-event projection once both prerequisites exist). Aggregate ~7-15 ed for full closure; partial closures (Axis C alone OR Axes A+B+D bundle OR Axis A alone) independently schedulable.
