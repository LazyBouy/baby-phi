...
The `phi` binary maps the envelope to stable exit codes so
...
### `PHI_SESSION_SECRET must be at least 32 bytes`

The session secret loaded from config (`session.secret` or the
`PHI_SESSION__SECRET` env var) is shorter than 32 bytes.
...
`PHI__SERVER__PORT=9090 phi-server`).
...
1. `phi-server` is up (`curl http://127.0.0.1:8080/healthz/live`).
2. `PHI_API_URL` (web-side) matches `server.host:server.port`
   (Rust-side).
...
1. Check the browser's DevTools → Storage → Cookies for
   `phi_kernel_session`. Absent? The claim response's `Set-Cookie`
   wasn't forwarded. Verify `PHI_SESSION_COOKIE_NAME`
   matches on both sides.
2. Check `PHI_SESSION_SECRET` is identical on the Rust
...
`data_dir`, `phi-server bootstrap-init`, retry. M7b adds
`phi admin regenerate-bootstrap-credential` for in-band
recovery.
