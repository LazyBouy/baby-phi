<!-- Last verified: 2026-05-04 by Claude Code (CH-12 amendment: the engine itself is unchanged — CH-12 lives at the validator module + Repository boundary, not at the engine. The publish-time precondition that the engine assumes (every persisted `ToolAuthorityManifest` has cleared `validate_published_manifest`) is now one rule stronger: CH-12 (ADR-0049) adds **Rule E** to the validator's rule set, rejecting `[Modify]` on composites whose `target_kinds` overlaps the reserved-namespace prefix list. The validator's rule set is now A/B/C/D/E (5 rules + 3 warnings) — Rule E composes with Rule C (Rule C still fires for `[Modify] × bare tag`; Rule E covers `[Modify] × composite × target_kinds`). Doc body UNCHANGED — the body does not enumerate the validator's rules.) -->
<!-- Last verified: 2026-05-03 by Claude Code (CH-11: Step 6's pseudocode "missing consent → Pending" is now one mapping in an 8-row response table. The real body at `domain::permissions::engine::step_6_consent_gating` branches on `winner.grant.approval_mode` × `ConsentPolicy` × `ConsentState`; `Acknowledged → Allow`, `Requested / not present → Pending`, `TimedOut → Allow|Denied(ConsentTimedOutDeny)` per org `approval_timeout_default_response`, `Declined / Revoked / Expired → Denied(ConsentX)`. New `DeniedReason` variants ship: `ConsentDeclined / ConsentRevoked / ConsentExpired / ConsentTimedOutDeny / NoSessionContext`. ADR-0048 ratifies; concept doc 06 §"Per-Session Consent" lines 297–349 + §"Per-policy mapping" 407–414 + §"Approval Timeout" 322 + line 349 default-response are typed Rust by this chunk. The high-level pipeline diagram body is unchanged.) -->
