...
Runtime configuration for `baby-phi-server` follows a **12-factor** pattern: non-secret defaults live in committed TOML files, environment-specific overlays layer on top, and secrets are injected via environment variables. Secrets never land in a file.
...
└─────────────────────────────────────────────────────────────────────┘
                                  ...
                                  ▼
┌─────────────────────────────────────────────────────────────────────┐
│ 2. config/{profile}.toml     — profile = $BABY_PHI_PROFILE or "dev" │
│                                committed; non-secret env tweaks     │
└─────────────────────────────────────────────────────────────────────┘
                                  │
                                  ▼
┌─────────────────────────────────────────────────────────────────────┐
│ 3. Environment variables      — prefix BABY_PHI_;                   │
...
- `BABY_PHI_PROFILE=dev` (default) → layers `config/dev.toml`.
- `BABY_PHI_PROFILE=staging` → layers `config/staging.toml`.
- `BABY_PHI_PROFILE=prod` → layers `config/prod.toml`.
...
|---|---|
| `server.host` | `BABY_PHI_SERVER__HOST` |
| `server.port` | `BABY_PHI_SERVER__PORT` |
| `server.tls.cert_path` | `BABY_PHI_SERVER__TLS__CERT_PATH` |
| `server.tls.key_path` | `BABY_PHI_SERVER__TLS__KEY_PATH` |
| `storage.data_dir` | `BABY_PHI_STORAGE__DATA_DIR` |
| `storage.namespace` | `BABY_PHI_STORAGE__NAMESPACE` |
| `storage.database` | `BABY_PHI_STORAGE__DATABASE` |
| `telemetry.log_filter` | `BABY_PHI_TELEMETRY__LOG_FILTER` |
| `telemetry.json_logs` | `BABY_PHI_TELEMETRY__JSON_LOGS` |
...
| Field | Type | Required | Default | Meaning |
|---|---|---|---|---|
| `data_dir` | path | yes | `"data/baby-phi.db"` | SurrealDB RocksDB data directory. |
| `namespace` | string | yes | `"baby-phi"` | SurrealDB namespace. |
...
- [`dev.toml`](../../../../../../config/dev.toml) — binds to `127.0.0.1`, uses `data/baby-phi-dev.db`, pretty logs, debug filter.
- [`staging.toml`](../../../../../../config/staging.toml) — binds `0.0.0.0`, `/var/lib/baby-phi/data`, JSON logs.
...
- `[M3]` OAuth 2.0 client credentials (`BABY_PHI_AUTH__OIDC__*`).
- `[M1/M7b]` SurrealDB at-rest encryption master key (`BABY_PHI_STORAGE__ENCRYPTION_KEY`).
- `[M7]` Audit-log off-site stream bucket/region (`BABY_PHI_AUDIT__STREAM__*`).
...
| Typo in env-var name (e.g. `BABY_PHI_SERVER_PORT` missing one underscore) | Env var silently ignored; field keeps layered value. This is a sharp edge — M7b adds a strict-mode validation pass in `ServerConfig::load()` that warns on unknown `BABY_PHI_*` env vars. |
| Unparseable value (e.g. non-numeric `BABY_PHI_SERVER__PORT`) | `try_deserialize()` returns `Err(ConfigError::Type)`; process exits. |
