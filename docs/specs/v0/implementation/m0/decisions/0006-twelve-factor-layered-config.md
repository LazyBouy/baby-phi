...
# ADR-0006: 12-factor layered config — TOML layers + `BABY_PHI_*` env overrides
...
baby-phi ships as a self-contained server that must run identically from a developer laptop through staging to production. Config needs are:
...
2. `config/{profile}.toml` — committed, profile-specific overrides. Profile selected by `BABY_PHI_PROFILE` (default `dev`; other values `staging`, `prod`).
3. Environment variables — `BABY_PHI_<SECTION>__<FIELD>` overrides any file value. Secrets **only** here.
...
- **Env-only overrides for debugging.** Any field can be flipped with a one-off env var (`BABY_PHI_TELEMETRY__LOG_FILTER=trace cargo run -p server`) without editing a committed file.
- **12-factor compliant.** Matches expectations of orchestrators, CI systems, and operators coming from other modern services.

### Negative

- **Env-var names are verbose.** `BABY_PHI_SERVER__TLS__CERT_PATH` is long. Mitigated by: the verbosity reflects real nesting; you only type them when setting overrides.
- **Silent typos.** A misspelled `BABY_PHI_SERVR__PORT` is ignored, not flagged. This is the `config` crate's default behaviour. M7b adds a strict-mode validator that warns on unknown `BABY_PHI_*` env vars — see [`../architecture/configuration.md`](../architecture/configuration.md) §"Failure modes".
...
Env examples:
- `BABY_PHI_PROFILE=prod`
- `BABY_PHI_SERVER__PORT=9090`
- `BABY_PHI_STORAGE__DATA_DIR=/mnt/ssd/baby-phi`
- `BABY_PHI_TELEMETRY__LOG_FILTER="info,server=debug"`
