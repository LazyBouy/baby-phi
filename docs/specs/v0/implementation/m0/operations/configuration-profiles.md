...
baby-phi-server selects a config profile at startup via `BABY_PHI_PROFILE`. The profile's TOML file overlays onto `config/default.toml`, and then environment variables override both. This page describes the three shipped profiles and how to add a new one.
...
[`config/dev.toml`](../../../../../../config/dev.toml). Selected when `BABY_PHI_PROFILE` is unset or `dev`.
...
- `storage.data_dir = "data/baby-phi-dev.db"` — keeps dev data out of the production default location.
...
[`config/staging.toml`](../../../../../../config/staging.toml). Selected when `BABY_PHI_PROFILE=staging`.

- `server.host = "0.0.0.0"` — bind all interfaces (typically one, inside a container network).
- `storage.data_dir = "/var/lib/baby-phi/data"` — persistent volume path.
- `telemetry.log_filter = "info"` — less noise.
- `telemetry.json_logs = true` — ingest-ready for Loki / Elasticsearch / Cloud Logging.

### `prod`

[`config/prod.toml`](../../../../../../config/prod.toml). Selected when `BABY_PHI_PROFILE=prod`. Set automatically in the Dockerfile.

Same shape as `staging`, with an extended comment documenting the TLS policy:

> The recommended production pattern is to terminate TLS at a reverse proxy in front of baby-phi-server, and run baby-phi-server itself over plaintext HTTP on an internal network. Native TLS via `[server.tls]` is supported for simple single-node deploys.
...
```bash
BABY_PHI_PROFILE=prod BABY_PHI_SERVER__PORT=9090 baby-phi-server
```
...
|---|---|---|---|---|
| `default.toml` | `8080` | `"0.0.0.0"` | `"data/baby-phi.db"` | `true` |
| `prod.toml` | *(inherits)* | *(inherits)* | `"/var/lib/baby-phi/data"` | `true` |
| env | **`9090`** | — | — | — |
| **Effective** | **9090** | `"0.0.0.0"` | `"/var/lib/baby-phi/data"` | `true` |
...
1. Create `config/canary.toml` with only the fields that differ from `default.toml`. Example:
   ...
   [storage]
   data_dir = "/var/lib/baby-phi/data"

   [telemetry]
   log_filter = "debug"   # chattier than prod so issues surface fast
   json_logs = true
   ```
2. Set `BABY_PHI_PROFILE=canary` in your deployment.
...
- `[M3]` `BABY_PHI_AUTH__OIDC__CLIENT_SECRET` — OAuth client secret.
- `[M1/M7b]` `BABY_PHI_STORAGE__ENCRYPTION_KEY` — 32-byte hex AES key for SurrealDB at-rest encryption.
- `[M7]` `BABY_PHI_AUDIT__STREAM__ACCESS_KEY` / `…SECRET_KEY` — write-only creds for the audit log off-site bucket.
...
The baby-phi server **does not** auto-load a `.env` file. Environment variables must be set in the process environment before `baby-phi-server` starts (e.g. by systemd, Docker `--env-file`, Kubernetes env, or a shell wrapper).

The baby-phi **CLI** does load `.env` (via the OpenRouter demo flow inherited from the phi-core scaffold). This is a developer convenience for running the CLI against a local API key; it's not a production pattern.
...
|---|---|
| Server listens on port 8080 despite `BABY_PHI_SERVER__PORT=9090` | Typo — single underscore between `SERVER` and `PORT`. Must be `__`. |
