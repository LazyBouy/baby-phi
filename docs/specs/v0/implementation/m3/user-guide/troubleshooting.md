...
| `ORG_NOT_FOUND` | 404 | GET /orgs/:id, GET /orgs/:id/dashboard | The org id is unknown to the store. Either the org was never created or the id typo'd. | Verify via `baby-phi org list` or `GET /orgs`. |
...
|---|---|---|
| All counters stuck at zero (including `token_budget.total`) | Org exists but `token_budget_pool` row is missing — only possible if the org was created outside `apply_org_creation` (manual SurrealDB insert) | `baby-phi org create` via the wizard, or `INSERT INTO token_budget_pool ...` manually matching the schema |
