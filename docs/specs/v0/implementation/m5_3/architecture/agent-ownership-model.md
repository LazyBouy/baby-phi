<!-- Last verified: 2026-05-18 by Claude Code (CH-27 / ADR-0062 P-DOCS — verified-header refresh: synth-owner-grant scope WIDENED to `[Allocate, Transfer, Observe, Inspect]` at CH-27 per ADR-0062 §D62.2; the 7 admin handlers now BLOCK via `denial_to_api_error` (CH-25 wire convention) per ADR-0062 §D62.1; D-CH26-FOLLOWUP-01 closed. §3 body amended to reflect 4-verb scope; the "Why [Allocate, Transfer]?" subsection retained with CH-27 widening note. Cycle hex `0edcaba9`.) -->
<!-- Last verified: 2026-05-17 by Claude Code (CH-26 P-DOCS — verified-header bump only: CH-26 ships the Composite-resource axis (`Composite::OrganizationObject` + `Composite::ProjectObject` variants) + per-instance tag pathway (`tags: Vec<String>` on Organization + Project with `organization:<uuid>` / `project:<uuid>` populated at-creation-time + via migration #0018 backfill) + advisory `check_permission` routing through 7 admin handlers. The synth-owner-grant rule documented in §3 below carries `[Allocate, Transfer]` Actions and is consumed advisorily at CH-26; CH-27 widens to `[Allocate, Transfer, Observe, Inspect]` + tightens to blocking gates per drift `D-CH26-FOLLOWUP-01`. See companion design page `composite-resources-model.md` + ADR-0061. Cycle hex `d1cb9e1f`.) -->
...
### Why `[Allocate, Transfer, Observe, Inspect]` on `IdentityPrincipal`?

**At CH-25** (per ADR-0060 §D60.2, F2.a user-lock): the forward-scope row literal *"[admin, transfer]"* re-interpreted to canonical `[Action::Allocate, Action::Transfer]` because `Action::Admin` does NOT exist in the canonical 34-verb set. Authority category at `action.rs:341-345` contains `Delegate`, `Approve`, `Escalate`, `Allocate`, `Transfer`. The 2-verb scope shipped at CH-25 covered the owner-tier Authority operations (allocate authority *of* the owned org/project to its child agents; transfer authority to peers).

**At CH-27** (per ADR-0062 §D62.2): the synth-grant scope is widened to **all 4 universal-applicability verbs** per `concepts/permissions/03-action-vocabulary.md:44` — `[Action::Allocate, Action::Transfer, Action::Observe, Action::Inspect]`. The widened scope adds the Discovery axis (`Inspect` — the natural verb for `show_*` handlers) and the Observability axis (`Observe` — the natural verb for `dashboard_*` + `list_*` handlers). The `IdentityPrincipal` fundamental class remains the axis on which all 4 verbs operate per concept-doc 03 lines 149 (Authority) + 195 (Authority for org/project tier) + 44 (universal-applicability claim covering Discovery + Observability).

This widening unblocks the CH-27 wire-tier tightening (ADR-0062 §D62.1): with all 4 verbs in scope, the 7 admin handlers can now propagate engine-deny as HTTP 403 `NO_GRANTS_HELD` for non-owner viewers without requiring explicit per-verb grants at every owner-test site.
