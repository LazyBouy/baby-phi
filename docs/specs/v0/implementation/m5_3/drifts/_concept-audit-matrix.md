...
| Org has Projects (Project = Resource Type) | Project + Organization are Composite resources subject to Permission Check selectors | honored | `Composite` enum cardinality 8 → 10 at `composites.rs` with NEW `OrganizationObject` + `ProjectObject` variants per ADR-0061 §D61.1 (CH-26 cycle hex `d1cb9e1f`); `tags: Vec<String>` field on Organization + Project (with `#[serde(default)]`) carries per-instance identity tag (`organization:<uuid>` / `project:<uuid>`) populated at-creation-time + via migration #0018 backfill; ≥ 15 advisory `check_permission` invocations across 7 admin handlers in `server/src/platform/{orgs,projects}/` close D-philosophy-02:39's ≥ 3-hit invariant; 10 acceptance scenarios at `acceptance_m5_3_composite_resources.rs` validate engine + handler-shape verdicts. Advisory → blocking-gate tightening + `projects::resolvers::*` actor-passthrough + synth-grant scope widening to `Inspect`+`Observe` carve out to CH-27. | **D-philosophy-02** (HIGH, **remediated at CH-26**); follow-up **D-CH26-FOLLOWUP-01** (LOW, CH-27 carve-out) |
