<!-- Status: § "Identity (Emergent, Event-Driven)" + § "Identity Node Content" + § "Materialization" [EXISTS] as of CH-16 (M5.2 / 2026-04-28); other sections still CONCEPTUAL -->
<!-- Last verified: 2026-04-28 by Claude Code (CH-16: Identity now materialized as 4-field struct per ADR-0038; § "Identity (Emergent, Event-Driven)" + § "Identity Node Content" + § "Materialization" all flip from contradicted to honored. § "Scoping the embedding model" + "Model change is an admin event" remain silent-in-code per ADR-0038 §D38.3 deferral to M6-DEFERRED-03. CH-21 amendment: § "Two Streams of Experience" `witnessed.memories_extracted` counter is now reactively bumped on every non-aborted `SessionEnded` for an LLM working agent — `MemoryExtractionListener` (CH-21 / ADR-0040 heuristic v0) calls `Repository::upsert_identity` with `+= 1` on the matching `extraction_scope_distribution.{private,public}` bucket. Supervisor-as-actor pattern (separate supervisor agent extracts from a subordinate session) deferred to M6-DEFERRED-04 per ADR-0040 §D40.5 — at v0 the working agent is its own extractor.) -->
