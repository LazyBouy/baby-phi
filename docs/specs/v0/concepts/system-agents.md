...
> **3-way union bucketing convention (CH-19 / drift D6.3, 2026-05-10).** The `GET /api/v0/orgs/:org/system-agents` list endpoint counts an Agent row as "standard" if **any** of three conditions holds: (a) the Agent's slug matches a canonical M5 slug (`memory-extraction-agent`, `agent-catalog-agent`); OR (b) the Agent's id appears in the owning org's `Organization.system_agents` registry array; OR (c) the Agent's `role` field is `AgentRole::System`. The 3-way union is robust against M3-era fixture rows that were created before the M5 canonical-slug convention existed (those rows lack the canonical slug but still carry `AgentRole::System` and/or registry membership). Implementation: [`server/src/platform/system_agents/list.rs`](../../../modules/crates/server/src/platform/system_agents/list.rs). Ratified at ADR-0057 §D57.3.
