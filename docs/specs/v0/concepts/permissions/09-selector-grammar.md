...
<!-- Last verified: 2026-05-04 by Claude Code (CH-12 amendment: §"Reserved Namespace Enforcement" lines 190–196 — CH-05's Rule C handled the bare-`tag` half of the read-vs-write asymmetry; CH-12 (ADR-0049) extends that enforcement to composites by adding Rule E to `validate_published_manifest`, which rejects `[Modify]` on a composite whose `target_kinds` overlaps `reserved_namespace_prefixes()`. Together CH-05 Rule C + CH-12 Rule E close both publish-time discriminators (bare-tag and composite-via-target_kinds). Drift D-new-08 closed. Doc body UNCHANGED.) -->
