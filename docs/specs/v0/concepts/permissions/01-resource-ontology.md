...
<!-- Last verified: 2026-05-17 by Claude Code (CH-26 P-DOCS amendment: Composite Classes table cardinality flipped 8 → 10 with NEW `organization_object` + `project_object` rows. Org/Project remain Principal-only at the Resource trait level (per CH-25 ADR-0060 §D60.1 + `principal_resource.rs:182-186`); the new Composite variants are categorical labels in the resource ontology + selector-match anchors. NEW `tags: Vec<String>` field on Organization + Project structs (with `#[serde(default)]`) carries the per-instance identity tag (`organization:<uuid>` / `project:<uuid>`) — see §"Instance Identity Tags" pathway below. The owner-grant authority for an Agent over their owned Org/Project continues to flow through the synth-owner-grant rule at `step_2_resolve_grants` per ADR-0060; CH-26 adds engine-routing through ≥ 7 admin handlers (advisory at CH-26; CH-27 tightens to blocking gates). See `m5_3/architecture/composite-resources-model.md` + ADR-0061 for the full design. Cycle hex `d1cb9e1f`.) -->
<!-- Last verified: 2026-05-16 by Claude Code (CH-25 amendment: Org and Project STAY Principal-only under F1.b user-lock. The new `Edge::Owns` variant uses a NEW `OwnedResourceId` enum payload to carry typed Org/Project ownership-target IDs WITHOUT relaxing the Resource trait — `principal_resource.rs:182-186`'s Principal-only invariant for OrgId/ProjectId is preserved. The owner-grant authority for an Agent over their owned Org/Project flows through the synth-owner-grant rule at `step_2_resolve_grants` per ADR-0060, not via `impl Resource for OrgId`. See `m5_3/architecture/agent-ownership-model.md` for the full design. Cycle hex `1e01618e`.) -->
...
| `organization_object` | `data_object` + `identity_principal` | `tag` + `#kind:organization` + per-instance tag `organization:<uuid>` (populated at-creation-time + via migration #0018 backfill) | Dual-nature: governance container (lifecycle / mission / vision / member-set) + first-class resource (the entity grants reference at the resource-axis). Org STAYS Principal-only at the Resource trait level (per CH-25 ADR-0060 §D60.1); the Composite variant is a categorical label + selector-match anchor. Engine routing through `check_permission` for ≥ 4 admin-page-1 handlers (Orgs admin): advisory at CH-26 close; CH-27 tightens to blocking. See [Composite resources model](../../implementation/m5_3/architecture/composite-resources-model.md). |
| `project_object` | `data_object` + `identity_principal` | `tag` + `#kind:project` + per-instance tag `project:<uuid>` | Same dual-nature framing as `organization_object`. Project STAYS Principal-only. Engine routing through `check_permission` for ≥ 3 admin-page-3 handlers (Projects admin): advisory at CH-26 close; CH-27 tightens to blocking + wires `projects::resolvers::*`. See [Composite resources model](../../implementation/m5_3/architecture/composite-resources-model.md). |
...
- All **10 composites** (`memory_object`, `session_object`, `external_service_object`, `model/runtime_object`, `control_plane_object`, `auth_request_object`, `inbox_object`, `outbox_object`, plus `organization_object` and `project_object` from [§Composite Classes](#composite-classes-10) — extended at CH-26 per ADR-0061 §D61.1 from 8 → 10 to carry the M5.3 §4.2 Org/Project-as-Composite-resources invariant).
