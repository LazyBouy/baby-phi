<!-- Status: CONCEPTUAL -->
<!-- Last verified: 2026-05-09 by Claude Code (CH-17 amendment: §"session lifecycle — live events" claim (operator observes via SSE) flips from contradicted-at-M5 to honored-at-CH-17. `GET /api/v0/sessions/:id/events` ships as a hard-deny-gated SSE handler at `server::handlers::sessions::events`; sibling permission-builder `domain::permissions::builders::build_session_observe_manifest` returns `[Observe]` on `session_object` (concept-doc 03 line 22 + line 44: Observability is universal across fundamentals — `Action::Observe` was already canonical pre-CH-17 at `action.rs:73,282,322`); recorder broadcast-tap on `BabyPhiSessionRecorder.broadcast_tx: Option<broadcast::Sender<phi_core::AgentEvent>>`; 30s SSE keep-alive; lagged-receiver → typed `event: lagged` SSE error then close. New `platform.session.live_stream_denied` (Alerted-class) audit event on every Step-1-to-5 deny. Migration `0016_template_a_session_object_grant_add_observe.surql` extends Template A's mint to `[Read, Inspect, List, Observe]` so legitimate launchers cover the SSE manifest (per ADR-0055 §D55.9). Drift D7.1 closed. ADR-0055 §D55.1/§D55.3/§D55.4/§D55.5/§D55.6/§D55.8/§D55.9. Doc body unchanged.) -->
<!-- Last verified: 2026-05-04 by Claude Code (CH-12 amendment: §"Frozen-at-creation tags (immutability)" lines 531–541 + Example 7 lines 516–525 lifted into typed Rust at `domain::permissions::manifest::validator::{validate_tag_write_on_session, CompositeStructuralTagWrite, SESSION_FROZEN_TAG_PREFIXES}` + `domain::audit::events::m5_2::tool_authority::frozen_tag_write_rejected` (Alerted-class). Publish-time validator extends CH-05 Rule C to a new Rule E: reject `[Modify]` on composites whose `target_kinds` overlaps the reserved-namespace prefix list. Runtime gate is a forward-defensive validator the Repository contract requires future tag-write methods to call; rejection produces an audit event per F5.B. Doc body unchanged.) -->
<!-- Last verified: 2026-04-28 by Claude Code (CH-21 amendment: §"Supervisor Extraction as Two Standard Grants" preserved silent-in-code at v0 — the heuristic listener (CH-21 / ADR-0040) bypasses grant enforcement and writes Memory rows via direct `Repository::create_memory` calls. Permission-checked extraction lands with the LLM-driven supervisor body in **M6-DEFERRED-04** per ADR-0040 §D40.4.) -->
