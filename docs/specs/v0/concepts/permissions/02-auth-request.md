<!-- Last verified: 2026-05-10 by Claude Code (CH-19 P1 — §"Interaction with Authority Templates" tail gains 1-paragraph refresh documenting multi-AR-per-(org, kind) history convention: an org may adopt → revoke → re-adopt the same template kind, producing multiple AR rows in history; `find_adoption_ar` returns the most-recent (sorted by `submitted_at` desc) per [`server/src/platform/templates/mod.rs`](../../../../modules/crates/server/src/platform/templates/mod.rs); ratified at ADR-0057 §D57.2 (closes drift D5.3). Doc body otherwise UNCHANGED. cycle hex `2c520ba7`.) -->
<!-- Last verified: 2026-05-08 by Claude Code (CH-14 amendment: §"System Bootstrap Template" lines 449–487 — the axiomatic root principal + bootstrap-AR identification + two-witness termination predicate are now typed Rust at `domain::permissions::axioms::{SYSTEM_GENESIS_PRINCIPAL, system_genesis_principal, is_bootstrap_ar, system_bootstrap_template_id}` per ADR-0053 §D53.1/§D53.2; bootstrap claim flipped to use the helper-fn at `bootstrap/claim.rs:192,204`; claim-time-as-system-init divergence from line 469's `fires_on: system_init` accepted per ADR-0053 §D53.6. Doc body unchanged.) -->
...
> **Multi-adoption history convention (CH-19 / drift D5.3, 2026-05-10).** An org may adopt a template, later revoke its adoption, and re-adopt the same template kind — producing **multiple adoption-AR rows for the same `(org_id, template_kind)` pair across history.** The "current adoption state" of a template kind for an org is the **most-recent AR's state** (sorted by `submitted_at` desc). The v0.1 server resolves this via `find_adoption_ar` at [`server/src/platform/templates/mod.rs`](../../../../modules/crates/server/src/platform/templates/mod.rs) — `.sort_by_key(|ar| Reverse(ar.submitted_at))` then `.first().cloned()`. The list-adoption endpoint preserves the full history (all rows visible for audit); the resolution endpoint surfaces only the current most-recent row. Ratified at ADR-0057 §D57.2.
