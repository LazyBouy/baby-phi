...
- Does NOT extend to **milestone-to-chunks decomposition** (the "M6+ phase-planning" you flagged — turning a future milestone into a forward-scope row list). The current `chunk-planner` operates from an existing forward-scope row. When M5 closes and M6 needs decomposition, the natural addition is a 5th agent (`phase-planner`, opus) that consumes a milestone goal + concept-doc set and emits forward-scope rows. Adding it then keeps the agent set within the user-approved 7–8 ceiling and avoids speculative design now. The 4-agent set is intentionally **shaped to be extensible** with this 5th role.
